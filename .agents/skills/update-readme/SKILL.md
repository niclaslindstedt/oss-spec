...
The file `.agents/skills/update-readme/.last-updated` contains the git commit hash from the last time the README was comprehensively updated. Use this as the baseline for discovering what changed.
...
- [ ] Update the **What `oss-spec validate` enforces** list if a §19 rule was added or reworded
...
- [ ] Run `oss-spec validate .` and make sure the repo still conforms
...
1. Read every edited section and confirm it matches the current source (`src/cli.rs`, `src/agent_help.rs`, `src/validate.rs`, `OSS_SPEC.md`).
